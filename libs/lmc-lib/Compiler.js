...
export default class Compiler {
  ...
  error;
  segments = {
    code: {CB: -1, CL: -1, mode: 'r-x'},
    data: {DB: -1, DL: -1, mode: 'r--'},
    free: {FB: -1, FL: -1, mode: 'rw-'}
  };
  ...
  constructor(text) {
    ...
    if (validator.error == undefined) {
      ...
      this.intermediate_code =
          this.getIntermediateCode(validator.text, this.labels);
  ...
  getSegments(intermediate_code) {
    ...
    this.segments.code.CL = this.segments.data.DB == -1 ?
        lines.length - 1 :
        this.segments.data.DB - 1;
